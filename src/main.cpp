...
#if defined(NDEBUG)
# error "Loopcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc6409285b9ab874ee447c7a1e7ff7c9e5e0dece51db0ba865b32d005c7bba423");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Loopcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Loopcoin Signed Message:\n";
...
{
    // Loopcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 0)
{
	nSubsidy = 29700000 * COIN;
}

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Loopcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Loopcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Loopcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Loopcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Loopcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5c4bb39399db5d6d39caaf2f2275a17406cdd756dda7f9824dbbf58b8a5bed6f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "We made a cryptooin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04705f85bea83146b5d13e3618f1cea7f4f1003ac9c1074e94b1dc45257a66113ce5b9c3d0241a5347b0164d0a1ea1b0defbd4fb7aae1408cd40944992a222a131") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Loopcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LoopcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LoopcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LoopcoinMiner(CWallet *pwallet)
{
    printf("LoopcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("loopcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LoopcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LoopcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LoopcoinMiner, pwallet));
}
